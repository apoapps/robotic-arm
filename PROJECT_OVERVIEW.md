...
Este proyecto controla un brazo robotico desde una PicoCalc con Raspberry Pi Pico 2 W. La PicoCalc corre una app en MicroPython, muestra una TUI en la terminal y al mismo tiempo levanta una red Wi-Fi local con una interfaz web para controlar el brazo desde Safari o cualquier navegador conectado a esa red.
...
El programa principal esta en:

```text
picocalc-lofifren/robotarm.py
```

Tambien se cargo un arranque automatico en:

```text
picocalc-lofifren/main.py
```

`main.py` ejecuta `robotarm` al iniciar la PicoCalc. Si el programa falla o se cierra, intenta regresar al menu de scripts de LofiFren.

La app se copia a la PicoCalc en:

```text
/sd/py_scripts/robotarm.py
```

## Firmware y entorno

La PicoCalc usa firmware LofiFren para PicoCalc con MicroPython en Raspberry Pi Pico 2 W.

Herramientas usadas en la computadora:
...
Para copiar la app por USB cuando la PicoCalc aparece como serial:

```sh
python3 -m mpremote connect /dev/cu.usbmodem11301 fs cp picocalc-lofifren/robotarm.py :/sd/py_scripts/robotarm.py
python3 -m mpremote connect /dev/cu.usbmodem11301 reset
```

En esta maquina el puerto normalmente aparece como:
...
- PicoCalc con Raspberry Pi Pico 2 W.
- Puente H para los motores DC y el actuador de 2 cables.
- Servo para la garra.
- Fuente externa de 5 V para motores y servo.
- GND comun entre PicoCalc, puentes H y fuente.

No alimentes motores ni servo desde `3V3`. Ese pin no es para carga de motores. Los motores y el servo deben usar una fuente externa. El GND de esa fuente si debe ir unido al GND de la PicoCalc.

## Cableado actual

Este es el mapeo que usa el codigo actual.

| Funcion | Pin PicoCalc | Conexion | Nota |
| --- | --- | --- | --- |
| Garra servo | `GP2` | Senal del servo | Rojo a 5 V externo, GND comun |
| Base | `GP4` / `GP5` | Entradas del puente H | Ya funcionaba, no se movio |
| Shoulder | `GP21` / `GP28` | Entradas del puente H | Ya funcionaba, no se movio |
| Elbow actuator | `GP3` / `UART0_RX` | Entradas del puente H | Actuador de 2 cables, reversible |

El actuador de elbow es de 2 cables, por eso va al puente H. Para avanzar y regresar necesita dos senales:

```text
GP3      -> IN3 del puente H
UART0_RX -> IN4 del puente H
Actuador -> OUT3 y OUT4 del puente H
```

Si se usara solo `GP3`, el actuador quedaria de un solo sentido. Por eso se agrego `UART0_RX` como segundo pin de control.

## Alimentacion

Cableado minimo de energia:

```text
Fuente 5 V +  -> +5V del puente H / V+ de servo
Fuente GND    -> GND del puente H
PicoCalc GND  -> GND del puente H
Servo GND     -> GND comun
Servo rojo    -> 5 V externo
Servo senal   -> GP2
```

El punto que no se puede saltar es el GND comun. Si el GND de la PicoCalc no esta unido al GND de la fuente externa, las senales de control pueden fallar aunque el programa este bien.

## Interfaz TUI en PicoCalc

La TUI muestra:

- Nombre del proyecto y equipo.
- Wi-Fi y password.
- URL local del servidor web.
- Lista de ejes con pin, nombre, valor y acciones.
- Estado actual.
...
| Tecla | Funcion |
| --- | --- |
| `UP` / `DOWN` | Selecciona eje |
| `LEFT` | Cierra garra o mueve eje hacia atras |
| `RIGHT` | Abre garra o mueve eje hacia adelante |
| `1` a `4` | Seleccion directa de eje |
| `S` / `Enter` | Stop |
| `Q` / `Esc` | Salir |

La TUI evita redibujar sin necesidad. Solo actualiza cuando cambia el estado, eje, valor o puerto web. Asi no parpadea tanto.

## Interfaz web

Al iniciar `robotarm.py`, la PicoCalc crea esta red:
...
La web funciona como centro de control compacto:

- Grilla 2x2 con los cuatro controles visibles.
- Cada tarjeta muestra pin, nombre y descripcion.
- Botones con flechas para mover cada eje.
- Banner superior con el eje seleccionado o el movimiento activo.
- Stop circular rojo.
- Sin dependencias externas ni CDN, para que funcione conectado al Wi-Fi de la PicoCalc.

Los botones web usan `onpointerdown`, asi que se pueden dejar presionados. Mientras el boton esta presionado, repite el movimiento cada 180 ms. Al soltar, manda `stop`.

La web consulta `/state` cada 2 segundos y usa `/cmd` para mandar acciones. Eso mantiene la web sincronizada con la TUI sin recargar la pagina.

## Ejes y acciones

| Eje | Pin(es) | Accion izquierda | Accion derecha |
| --- | --- | --- | --- |
| Gripper | `GP2` | Close | Open |
| Base motor | `GP4/GP5` | Back | Forward |
| Shoulder motor | `GP21/GP28` | Back | Forward |
| Elbow actuator | `GP3/UART0_RX` | Back | Forward |

La garra usa un paso mas grande para que no se sienta lenta:
...
La app usa un servidor HTTP pequeno hecho con `socket`.

| Ruta | Uso |
| --- | --- |
| `/` | Devuelve la interfaz web |
| `/state` | Devuelve JSON con eje, nombres, valores y estado |
| `/cmd?a=axis&i=0..3` | Cambia eje seleccionado |
| `/cmd?a=move&i=0..3&d=-1/1` | Mueve un eje |
| `/cmd?a=stop` | Apaga salidas |
...
```json
{
  ...
  "names": ["Gripper", "Base motor", "Shoulder motor", "Elbow actuator"],
  "manual": [90, 90, 90, 90],
  "status": "Ready"
}
```
...
| Archivo | Uso |
| --- | --- |
| `picocalc-lofifren/robotarm.py` | App principal del robot |
| `picocalc-lofifren/main.py` | Arranque automatico hacia `robotarm` |
| `tools/update_lofifren_app_usb.sh` | Copia la app por USB usando `mpremote` |
| `tools/requirements.txt` | Dependencias de computadora |
| `README.md` | Estructura general del repo |

## Codigo completo: `picocalc-lofifren/main.py`
...
## Codigo completo: `picocalc-lofifren/robotarm.py`
...
axis_desc = ("Claw open / close", "Rotate base", "Lift arm", "Elbow back / forward")
...
<div class="info"><span>{pin}</span><b>{name}</b><em>{desc}</em><strong id="v{i}">{value}</strong></div>
<div class="pair"><button class="jog" onpointerdown="holdAxis({i},-1);return false" onpointerup="releaseHold()" onpointercancel="releaseHold()" onpointerleave="releaseHold()">{neg}</button><button class="jog" onpointerdown="holdAxis({i},1);return false" onpointerup="releaseHold()" onpointercancel="releaseHold()" onpointerleave="releaseHold()">{pos}</button></div>
</section>""".format(active=active, i=i, label=labels[i], pin=pin_names[i], name=axis_names[i], desc=axis_desc[i], value=manual[i], neg=action_button_label(i, -1), pos=action_button_label(i, 1), axis_icon=axis_icon(i))
...
main{{height:100dvh;min-height:0;overflow:hidden;padding:6px;display:grid;grid-template-rows:auto 1fr auto;gap:6px}}
.brand{{display:grid;grid-template-columns:30px 1fr auto;gap:8px;align-items:center}}.logo{{width:30px;height:30px;border:2px solid #000;display:grid;place-items:center}}
.title{{font-size:15px;font-weight:900;text-transform:uppercase;line-height:1.05}}.sub{{font-size:11px;margin-top:1px}}
.ion{{width:20px;height:20px;vertical-align:-5px}}details{{position:relative}}summary{{list-style:none;border:2px solid #000;padding:7px;display:flex;gap:5px;align-items:center;font-weight:900;font-size:12px}}summary::-webkit-details-marker{{display:none}}
...
button{{border:2px solid #000;border-radius:0;background:#fff;color:#000;font:inherit;padding:8px;font-weight:900;min-height:42px;display:flex;align-items:center;justify-content:center;gap:6px;touch-action:none;user-select:none;-webkit-user-select:none}}
button:active{{background:#000;color:#fff}}
.grid{{min-height:0;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;gap:6px}}.axis{{min-height:0;border:2px solid #000;padding:5px;display:grid;grid-template-columns:48px 1fr;grid-template-rows:auto 1fr;gap:4px;align-items:center}}
.axis .pair{{grid-column:1/3;align-self:end}}.axis strong{{float:right;font-size:15px}}.axis b{{display:block;font-size:12px;line-height:1}}.axis span{{font-size:10px}}.axis em{{display:block;font-style:normal;font-size:9px;line-height:1.05}}.pick{{padding:4px;min-height:38px;flex-direction:column;gap:0;font-size:10px}}.selected{{background:#000;color:#fff;outline:2px solid #000;outline-offset:1px}}.selected button{{border-color:#fff}}
.moves,.pair{{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin:0}}.moves{{display:none}}.pair .jog{{font-size:11px;min-height:38px;padding:4px 2px}}.pair .jog .ion{{width:16px;height:16px}}.hero{{border:3px solid #000;padding:5px;text-align:center;background:#fff;color:#000;display:grid;grid-template-columns:1fr auto;gap:6px;align-items:center}}.hero .small{{font-size:10px;text-transform:uppercase}}.hero .big{{font-size:18px;font-weight:900}}.moving{{background:#000;color:#fff}}.moving .big{{font-size:20px}}.activeMove{{background:#000!important;color:#fff!important}}.panic{{width:64px;height:64px;border-radius:999px;background:#c00;color:#fff;border:4px solid #000;margin:0 auto;font-size:11px;flex-direction:column}}.panic:active{{background:#900;color:#fff}}.meta{{display:none}}
@media (max-height:620px){{main{{gap:4px;padding:4px}}.hero{{padding:4px}}.hero .big{{font-size:16px}}.axis{{padding:4px;gap:3px}}.pair .jog{{min-height:34px;font-size:10px}}.panic{{width:54px;height:54px}}}}
</style></head><body>
<main><div id="hero" class="hero"><div><div class="small">Proyecto final Robotica</div><div id="heroText" class="big">{axis}</div></div><b id="status">{status}</b></div><div class="meta"><div>Axis <b id="axisName">{axis}</b></div><div>Pulse <b>{pulse} ms</b></div></div>
...
<div class="grid">{rows}</div><button class="panic" onclick="stopAll()">{stop}<span>Stop</span></button></main>
<script>
